...
char rbf_file [32];//"fpga_config_file.rbf" (edited for generic RBF filename);

int main(int argc, const char * argv[])
{
  //Set RBF filename (New Sección)
  if(strcmp(argv[1], "config_fpga") == 0 && argc < 3) {
    printf("missing RBF filename\n");
    exit(1);
  }
  if (argc == 3){
    int lenFilename = strlen(argv[2]);
    if (lenFilename >= sizeof(rbf_file)) {
      printf("filename too long: %s\n", argv[2]);
      exit(1);
    }
    strcpy(rbf_file, argv[2]);
  }
  printf("test_duver argc = %d\n", argc);
